...
class OrderReceiptTests(TestCase):
    ...
    def test_order_receipt(self):
        ...
        rsp = self.client.get(f'/payments/payment/{self.invoice.payment.id}/', follow=True)
        ...
        self.assertContains(rsp, '''
            <tr>
                <td>9A19</td>
                <td>PyCon UK 2018 Ticket for alice-1@example.com (Saturday, Sunday, Monday)</td>
                <td>1</td>
                <td>&pound;105.00</td>
                <td>20%</td>
                <td>&pound;126.00</td>
    ...
    def test_when_not_authenticated(self):
        rsp = self.client.get(f'/payments/payment/{self.invoice.payment.id}/', follow=True)
        self.assertRedirects(rsp, f'/accounts/login/?next=/payments/payment/{self.invoice.payment.id}/')
